...
const PlaceBanner = ({ eventData }) => {
  const bannerImageUrl = eventData.bannerImageUrl || 
    "https://assets-in.bmscdn.com/nmcms/events/banner/desktop/media-desktop-chaman-chatora-gaurav-kapoor-s-comedy-special-0-2024-5-29-t-13-6-22.jpg";
    ...
    <div className="flex flex-col">
      ...
      <div className="p-4 shadow-md">
        <div className="flex justify-between items-center">
          <div className="flex flex-col gap-2">
            <h1 className="text-lg md:text-xl font-bold">
              {eventData.placeName || "Event Title"}
            </h1>
            <p className="text-gray-600 text-sm">
              {eventData.description || "Event Category | Language | Age Limit | Duration"}
            </p>
            <div className="flex gap-4 items-center text-sm">
              <p className="text-gray-700">
                {eventData.fromDate || "Date"} - {eventData.toDate || "Date"}
              </p>
              <p className="flex items-center gap-2 text-gray-700 border-r-2 px-6">
                <FaLocationDot className="text-yellow-600" /> 
                {eventData.location?.locationName || "Event Location"}
              </p>
              <div className="text-red-600 font-semibold">
                ₹ {eventData.ticketPrice || "Price"} onwards
              </div>
            </div>
          </div>
